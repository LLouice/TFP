...
def load_graphdata_channel1(dataset_name,
                            num_of_hours,
                            num_of_days,
                            num_of_weeks,
                            DEVICE,
    ...
    file = os.path.basename(dataset_name).split('.')[0]

    dirpath = os.path.dirname(dataset_name)

    filename = os.path.join(
        dirpath, file + '_r' + str(num_of_hours) + '_d' + str(num_of_days) +
        '_w' + str(num_of_weeks))
    ...
    # ------- test_loader -------
    test_x_tensor = torch.from_numpy(test_x).type(torch.FloatTensor).to(
        DEVICE)  # (B, N, F, T)
...
def load_graphdata_channel_my(filename,
                              num_of_hours,
                              num_of_days,
                              num_of_weeks,
                              DEVICE,
